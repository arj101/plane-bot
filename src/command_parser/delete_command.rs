...
pub fn delete(msg: &serenity::model::channel::Message, ctx: &serenity::client::Context) {
    ...
    println!(
        "Got custom command delete request from : {}",
        msg.author.name
    );
    ...
    match member {
        Some(_) => {
            ...
            let guild = &ctx
                .http
                .get_guild(msg.guild_id.unwrap_or_default().0)
                .unwrap();

            for (_key, value) in &guild.roles {
                if value.permissions.administrator()
                    && msg.author.has_role(&ctx.http, guild.id, value).expect(":|")
                {
                    has_perm = true;
                };
            }
            ...
            if has_perm {
                ...
                if let Ok(key) = env::var("FIREBASE_API_KEY") {
                    ...
                    if let Some(cmd) = cmd {
                        ...
                        if let Err(why) = delete_custom_command_half_front_end(
                            &msg,
                            &ctx,
                            cmd,
                            auth_token.as_ref().expect("Error while reading AUTH_TOKEN"),
                        ) {
                            ...
                            if let Err(why) = delete_custom_command_half_front_end(
                                ...
                                if let Err(why) = msg.channel_id.say(
                                    &ctx.http,
                                    format!("Error deleting custom command: `{}`", why),
                                ) {
                        ...
                        if let Err(why) =
                            msg.channel_id.say(&ctx.http, "Command to remove not found")
                        {
                            println!("Error sending message: {:?}", why);
                        };
                    }
                    ...
                    if let Err(why) = msg
                        .channel_id
                        .say(&ctx.http, "api key not found in environment variables")
                    {
                        println!("Error sending message: {:?}", why);
                    };
                }
                ...
                if let Err(why) = msg.channel_id.say(
                    &ctx.http,
                    "You dont have permission to delete a custom command!",
                ) {
                    println!("Error sending message: {:?}", why);
                };
            }
...
fn delete_custom_command(command: &str, auth: &str) -> Result<(), String> {
    let resp = ureq::patch(&format!(
        "https://plane-bot.firebaseio.com/commands/{}.json?auth={}",
        command, auth
    ))
    .send_json(ureq::json!(serde_json::Value::Null));
    if resp.ok() {
        Ok(())
    } else {
        ...
        Err(format!(
            "Error while sending PATCH request to database. Status code:  {}. Response: {}",
            status_code, response
        ))
    }
}

fn delete_custom_command_half_front_end(
    ...
    match resp {
        Ok(_) => {
            ...
            if let Err(why) = msg.channel_id.say(
                &ctx.http,
                format!("Succesfully deleted custom command: `{}` !", cmd),
            ) {
                println!("Error sending message: {:?}", why);
            }
...
fn member_is_admin(
    ...
    match &msg.member {
        Some(_) => match &ctx.http.get_guild(msg.guild_id.unwrap_or_default().0) {
            Ok(guild_partial) => {
                let mut has_perm = false;
                ...
                for (_key, value) in &guild_partial.roles {
                    if value.permissions.administrator() {
                        if let Ok(admin_perm) =
                            msg.author.has_role(&ctx.http, guild_partial.id, value)
                        {
                            has_perm = admin_perm;
                        } else {
                            err = true;
                ...
                if err {
                    return Err(String::from(
                        "Error while checking user has admin roles or not",
                    ));
                } else {
                    Ok(has_perm)
                }
