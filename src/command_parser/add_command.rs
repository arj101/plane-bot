        ...
        if cmd_reply_formattted.len() >= 1 {
            match env::var("FIREBASE_API_KEY") {
                Ok(api_key) => {
                    ...
                    if let Err(why) = add_custom_command_half_front_end(
                        &msg,
                        &ctx,
                        cmd,
                        &cmd_reply_formattted,
                        auth_token.as_ref().expect("Error while reading auth token"),
                    ) {
                        println!(
                            "Error adding custom command,{}\nTrying again with new auth token! ",
                            why
                        );
                        ...
                        if let Err(why) = add_custom_command_half_front_end(
                            ...
                            if let Err(why) = msg
                                .channel_id
                                .say(&ctx.http, format!("Error adding custom command: `{}`", why))
                            {
                ...
                Err(why) => {
                    ...
                    if let Err(why) = msg.channel_id.say(
                        &ctx.http,
                        "Cannot find firebase access token in environment variables.",
                    ) {
                        println!("Error sending message: {:?}", why);
                    };
                }
            ...
            if let Err(why) = msg
                .channel_id
                .say(&ctx.http, "Reply for command to add not found!")
            {
                println!("Error sending message: {:?}", why);
            };
        }
...
fn add_custom_command_half_front_end(
    ...
    match resp {
        Ok(_) => {
            ...
            if let Err(why) = msg
                .channel_id
                .say(&ctx.http, "Succesfully added custom command!")
            {
                println!("Error sending message: {:?}", why);
            }
...
fn add_custom_command(command: &str, reply: &str, auth: &str) -> Result<(), String> {
    ...
    .send_json(ureq::json!({ command: reply }));
    if resp.ok() {
        Ok(())
    } else {
