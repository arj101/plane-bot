...
fn refresh_auth_token(api_key: &str) -> Result<String, String> {
    ...
    let id_token_resp = ureq::post(&format!(
        "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key={}",
        api_key
    ))
    .set("Content-Type", "application/json")
    .send_json(ureq::json!({"returnSecureToken":true}));
    ...
    if id_token_resp.ok() {
        ...
        return Err(format!(
            "Error in new token request, server didnt respod with OK"
        ));
    }
}
